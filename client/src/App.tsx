...
export function App() {
  ...
  const ocrForUser = useMemo<IdentityOcrResult>(
    () => ({
      ...ocrFixtures.driversLicenseClear,
      firstName: identity.firstName || "Lakshya",
      lastName: identity.lastName || "Bhambhani",
      dateOfBirth: identity.dateOfBirth || "1998-04-15"
    }),
  ...
  function validateProfile() {
    const result = validateW2Profile({ ocr: ocrForUser, profile, duplicateSsns });
...
function InstaworkOnboardingScreen({
  ...
  identityDocument: IdentityOcrResult;
  ...
  if (step === 6) {
    return <GovernmentIdUploadVerificationScreen profile={profile} onNext={onNext} onBack={onBack} />;
  }
  return (
...
function GovernmentIdUploadVerificationScreen({
  profile,
  onNext,
  onBack
}: {
    ...
    try {
      ...
      setDocuments((current) => ({
        ...
        [side]: {
          ...
          analysis: null,
          status: "error",
          message: error instanceof Error ? error.message : "Identity OCR failed. Try another image."
        }
      }));
    }
  }
      ...
      <div className="government-id-content">
        ...
        {blockingAnalysis && (
          <div className="id-feedback error" role="alert">
            ...
            <button className="id-inline-action" onClick={onBack}>Go back to edit profile</button>
          </div>
        )}
...
function GovernmentIdVerificationScreen({
  profile,
  identityDocument,
  onNext,
  onBack
  ...
  const [capturedDocumentLabel, setCapturedDocumentLabel] = useState("");
  const [validationIssues, setValidationIssues] = useState<string[]>([]);
  const [analysis, setAnalysis] = useState<IdentityVerificationAnalysis | null>(null);
  ...
  async function openCamera(facing?: "environment" | "user") {
    const targetFacing = facing ?? cameraFacing;
    setCapturedIdImage("");
    setCapturedDocumentLabel("");
    setValidationIssues([]);
    setAnalysis(null);
    cameraStream?.getTracks().forEach((track) => track.stop());
    if (!navigator.mediaDevices?.getUserMedia) {
      setFeedback({
        status: "error",
        message: "Camera access is unavailable in this browser. Use a device with camera permissions enabled."
      });
      return;
    }
    ...
    try {
      ...
      setFeedback({
        status: "idle",
        message: "Camera ready. Place the full US ID inside the frame with sharp, readable text."
      });
    } catch {
      setFeedback({
        status: "error",
        message: "Couldn’t access camera. Please allow access to your device’s camera."
      });
    }
  }
  ...
  function isLikelyBlurry(context: CanvasRenderingContext2D, width: number, height: number) {
    if (!("getImageData" in context)) return false;

    try {
      const data = context.getImageData(0, 0, width, height).data;
      let contrastScore = 0;
      for (let index = 0; index < data.length - 16; index += 64) {
        const current = data[index] + data[index + 1] + data[index + 2];
        const next = data[index + 16] + data[index + 17] + data[index + 18];
        contrastScore += Math.abs(current - next);
      }
      return contrastScore < 500;
    } catch {
      return false;
    }
  }

  function hasDocumentLikeFrame(context: CanvasRenderingContext2D, width: number, height: number) {
    if (!("getImageData" in context)) return true;

    try {
      const data = context.getImageData(0, 0, width, height).data;
      const left = Math.floor(width * 0.16);
      const right = Math.floor(width * 0.84);
      const top = Math.floor(height * 0.28);
      const bottom = Math.floor(height * 0.7);
      const inset = Math.max(6, Math.floor(Math.min(width, height) * 0.02));
      const step = Math.max(4, Math.floor(Math.min(width, height) / 70));
      ...
      for (let x = left; x <= right; x += step) {
        borderSamples.push(brightnessAt(x, top), brightnessAt(x, bottom));
      }
      for (let y = top; y <= bottom; y += step) {
        borderSamples.push(brightnessAt(left, y), brightnessAt(right, y));
      }
      for (let y = top + inset; y <= bottom - inset; y += step * 2) {
        for (let x = left + inset; x <= right - inset; x += step * 2) {
          innerSamples.push(brightnessAt(x, y));
        }
      }
      for (let y = 0; y < height; y += step * 3) {
        for (let x = 0; x < width; x += step * 3) {
          const insideCandidate = x >= left && x <= right && y >= top && y <= bottom;
          if (!insideCandidate) {
            outsideSamples.push(brightnessAt(x, y));
          }
        }
      }

      const borderMean = average(borderSamples);
      const innerMean = average(innerSamples);
      const outsideMean = average(outsideSamples);
      const borderToInnerContrast = Math.abs(borderMean - innerMean);
      const documentToBackgroundContrast = Math.abs(innerMean - outsideMean);

      // A random busy camera scene has plenty of edges, but it usually does not
      // have one large, card-shaped rectangle with a distinct border and body.
      return borderToInnerContrast > 45 && documentToBackgroundContrast > 25;
    } catch {
      return true;
    }
  }

  function selectedDocumentLabel(value: GovernmentIdType | "" = documentType) {
    if (!value) return "Selected document";
    return governmentIdTypeLabel(value);
  }

  function ocrForDetectedVisualDocument(selectedType: GovernmentIdType, documentInFrame: boolean): IdentityOcrResult {
    if (documentInFrame && selectedType === "permanent-resident-card") {
      return {
        ...identityDocument,
        documentType: "permanent_resident_card",
        selectedDocumentType: "permanent_resident_card",
        ...
        confidence: 0.82,
        imageQuality: identityDocument.imageQuality
      };
    }
  ...
  function buildFriendlyIssues(result: IdentityVerificationAnalysis): string[] {
    const messages: string[] = [];
    const vr = result.validationResults ?? {} as IdentityVerificationAnalysis["validationResults"];
    const nameStatus = (vr.nameMatch ?? { status: "NOT_CHECKED" }).status;
    const dobStatus = (vr.dobMatch ?? { status: "NOT_CHECKED" }).status;
    ...
    if (dobStatus === "MISMATCH") {
      const dm = vr.dobMatch;
      messages.push(
        dm?.details
          ? `Date of birth mismatch: ${dm.details}`
          : `The date of birth on your document does not match your profile. Go back and correct your date of birth.`
      );
    }

    for (const flag of (result.flags ?? [])) {
      if (flag.code === "NAME_MISMATCH" || flag.code === "DOB_MISMATCH") continue;
      if (flag.code === "BACK_IMAGE_REQUIRED" || flag.code === "EXPIRATION_DATE_NOT_FOUND" || flag.code === "NO_DOCUMENT_DETECTED" || flag.code === "PHOTO_BLURRED") continue;
      if (flag.code === "DOCUMENT_TYPE_MISMATCH") {
        messages.push(`Document type mismatch: you selected ${result.userSelectedTypeLabel}, but the image shows ${result.detectedDocumentTypeLabel}.`);
        continue;
      }
      if (flag.code === "DOCUMENT_EXPIRED") {
        messages.push("This document has expired. Please use a current, non-expired ID.");
        continue;
      }
  ...
  async function captureIdPhoto() {
    if (!documentType) {
      setFeedback({
        status: "error",
        message: "Choose the US government ID type before capturing."
      });
      return;
    }
    ...
    if (!video || !canvas || !context || !cameraStream) {
      setFeedback({
        status: "error",
        message: "Open the camera and center the US government ID before capturing."
      });
      return;
    }
    ...
    const documentInFrame = hasDocumentLikeFrame(context, width, height);
    const blurredCapture = isLikelyBlurry(context, width, height);
    const detectedOcr = ocrForDetectedVisualDocument(documentType as GovernmentIdType, documentInFrame);
    const ocrEffective: IdentityOcrResult = {
      ...detectedOcr,
      imageQuality: blurredCapture ? "blur" : detectedOcr.imageQuality
    };

    cameraStream.getTracks().forEach((track) => track.stop());
    setCameraStream(null);
    setCapturedIdImage(capturedImage);
    setIsAnalyzingId(true);

    let pipelineResult: IdentityVerificationAnalyzeResponse;
    try {
      const response = await fetch("/api/identity-verification/analyze", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({
          requestId: `identity_${Date.now()}`,
          imageBase64: capturedImage,
          selectedDocumentType: documentType,
          documentSide,
          documentDetectedInFrame: documentInFrame,
          profile
        })
      });
      ...
      if (!response.ok) {
        const errorBody = (await response.json().catch(() => null)) as { error?: string } | null;
        throw new Error(errorBody?.error || `Identity analysis failed with ${response.status}`);
      }

      pipelineResult = (await response.json()) as IdentityVerificationAnalyzeResponse;
    } catch (error) {
      setAnalysis(null);
      setValidationIssues([]);
      setFeedback({
        status: "error",
        message: error instanceof Error ? error.message : "Identity verification service is unavailable. Please try again."
      });
    ...
    const result = pipelineResult.analysis;
    setAnalysis(result);
    const issues = buildFriendlyIssues(result);
    ...
    const captureLabel =
      documentSide === "back"
        ? `${result.detectedDocumentTypeLabel} back side`
        : result.detectedDocumentTypeLabel;
    setCapturedDocumentLabel(captureLabel);

    if ((result.flags ?? []).some((flag) => flag.code === "PHOTO_BLURRED")) {
      setFeedback({
        status: "error",
        message: pipelineResult.userMessage
      });
      return;
    }

    if (!result.complianceEligibility) {
      setFeedback({
        status: "error",
        message: pipelineResult.userMessage
      });
  ...
  return (
    <section className="government-id-screen">
      ...
      <div className="government-id-content">
        <BackButton onClick={onBack} />
        <h1>Government ID</h1>
        <p>Take a clear real-time photo of the front of your US government ID.</p>
        <label className="id-type-field">
          US government ID type
          <select
            value={documentType}
        ...
        <label className="id-type-field">
          ...
          <select
            value={documentSide}
            onChange={(event) => {
              setDocumentSide(event.target.value as DocumentSide);
              setFeedback({ status: "idle", message: "" });
              setValidationIssues([]);
              setCapturedIdImage("");
              setCapturedDocumentLabel("");
        ...
        <p className="id-side-guidance">
          {documentSide === "front"
            ? "Front OCR checks document type, name, date of birth, photo, and address when present."
            : "Back OCR checks barcode/machine-readable data and must match the front/profile details."}
        </p>
        <div className={`government-id-preview ${cameraStream ? "live" : ""}`}>
          {capturedIdImage ? (
            <img src={capturedIdImage} alt="Captured government ID" />
          ) : cameraStream ? (
            <video ref={videoRef} className="government-id-video" aria-label="Live government ID camera preview" playsInline muted />
          ) : (
            <div className="id-card-icon" aria-hidden="true">
              <span />
              <i />
        ...
        <div className="id-camera-controls">
          <button className="id-camera-button" onClick={() => openCamera()}>Open camera</button>
          {cameraStream && (
            <button className="id-camera-toggle" onClick={toggleCamera} aria-label="Switch camera">
              {cameraFacing === "environment" ? "Switch to front camera" : "Switch to back camera"}
            </button>
          )}
        </div>
        <button className="id-capture-button" onClick={captureIdPhoto} disabled={isAnalyzingId}>
          {isAnalyzingId ? "Analyzing ID..." : "Capture ID photo"}
        </button>
        <canvas ref={canvasRef} className="visually-hidden" aria-hidden="true" />
        {capturedDocumentLabel && (
          <div className="captured-id-summary">
            <strong>Exact captured image</strong>
            <span>Captured image detected: {capturedDocumentLabel}</span>
          </div>
        )}
        {validationIssues.length > 0 && (
          <ul className="id-issue-list">
            {validationIssues.map((issue) => (
              <li key={issue}>{issue}</li>
            ))}
          </ul>
        )}
        {analysis && analysis.documentDetected && (
          <AnalysisPanelBoundary><IdentityAnalysisPanel analysis={analysis} /></AnalysisPanelBoundary>
        )}
        {feedback.message && (
          <div className={`id-feedback ${feedback.status}`} role={feedback.status === "error" ? "alert" : "status"}>
            {feedback.message}
          </div>
        )}
      </div>
      <div className="government-id-footer">
        <button className="blue-cta" onClick={onNext} disabled={feedback.status !== "success" || isAnalyzingId}>Continue</button>
      </div>
    </section>
  );
}
...
function I9DocumentUpload({
  ...
  async function analyzeDocImage(imageKey: string, govType: GovernmentIdType, imageBase64: string, fileName: string, side: "front" | "back", docEntry?: typeof docsToUpload[0]) {
    ...
    try {
      ...
      if (side === "back") {
        ...
        if (detectedSide === "front") {
          updateDocImage(imageKey, {
            ...
            message: "This looks like the front side of the document. Please upload the back side."
          });
        } else {
          updateDocImage(imageKey, {
      ...
      if (side === "back") {
        updateDocImage(imageKey, { imageBase64, fileName, analysis: null, status: "success", message: "Back side captured." });
        return;
      }
      updateDocImage(imageKey, {
        imageBase64,
        ...
        analysis: null,
        status: "error",
        message: error instanceof Error ? error.message : "Analysis failed. Try another image."
      });
    }
  }
