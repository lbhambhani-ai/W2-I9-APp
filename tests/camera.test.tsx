...
describe("real camera onboarding step", () => {
  ...
  it("does not offer under-18 years in the custom date picker", async () => {
    ...
    expect(screen.queryByText(under18Year, { selector: "option" })).not.toBeInTheDocument();
    const anyEnabledDay = document.querySelector(".calendar-days button:not(:disabled)");
    expect(anyEnabledDay).toBeInstanceOf(HTMLButtonElement);
    await userEvent.click(anyEnabledDay as HTMLButtonElement);
  ...
  it("uses a custom date picker instead of the browser default calendar", async () => {
    ...
    expect(document.querySelector(".calendar-days")).toBeInTheDocument();
  });
});
